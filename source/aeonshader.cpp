...
namespace aeon
{
    ...
    GLuint initProgram(string vertexFile, string fragmentFile, string defaultVertex, string defaultFragment)
    {
        // Create shaders
        GLuint vertexShaderID = glCreateShader(GL_VERTEX_SHADER);
        GLuint fragmentShaderID = glCreateShader(GL_FRAGMENT_SHADER);
        ...
        {
            log("Failed to open vertex shader at: \""+tempV+"\"", AEON_WARNING);
            log("Using default vertex shader.", AEON_WARNING);
            vertexCode = defaultVertex;
            ofstream vertexShaderStreamO(tempV.c_str(), std::ios::out);
            if(vertexShaderStreamO.is_open())
            {
                vertexShaderStreamO << vertexCode;
                vertexShaderStreamO.close();
            }
            else
            {
                log("Failed to write vertex shader at: \""+tempV+"\"", AEON_ERROR);
            }
        }
        ...
        {
            log("Failed to open fragment shader at: \""+tempF+"\"", AEON_WARNING);
            log("Using default fragment shader.", AEON_WARNING);
            fragmentCode = defaultFragment;
            ofstream fragmentShaderStreamO(tempF.c_str(), std::ios::out);
            if(fragmentShaderStreamO.is_open())
            {
                fragmentShaderStreamO << fragmentCode;
                fragmentShaderStreamO.close();
            }
            else
            {
                log("Failed to write fragment shader at: \""+tempF+"\"", AEON_ERROR);
            }
        }
