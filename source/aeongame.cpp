...
namespace aeon
{
    ...
    void SquareState::load(Config * settings, GLFWwindow* window, Context * context)
    {
        // TODO: InputHandler
        aSettings = settings;
        aInput = TestInput();
    ...
    void SquareState::loadGL()
    {
        ...
        programID = initProgram( "vert2.glsl", "frag2.glsl", vShader, fShader );

        if(programID == 0)
        {
            // NOTE: I'm pretty sure this is dangerous atm, but the program shouldn't fail to load anyway.
            throw "Failed to load program for SquareState";
        }
    ...
    void SquareState::processInput()
    {
        ...
        {
            log(e.what(), AEON_ERROR);
        }
        try
        {
            if(aInput.isMouseDown("LMB"))
            {
                State * temp;
                temp = new TestState;
                changeState(temp);
    ...
    void SquareState::update()
    {
        // Does nothing atm
    }
    void SquareState::render()
    {
