...
namespace aeon
{
    ...
    void computeCameraMatrices()
    {
        ...
        if(isMouseDown("RMB") && adjustingView)
        {
            ...
            glfwGetWindowSize(getMainWindowHandle(),&windowWidth,&windowHeight);
        ...
        // Deprecated as of glfw 3, must use scrollwheel callbacks now.
        //FoV -= 5 * getMouseWheel();     // Adjust Field of view based on scrolling
        //setMouseWheel();                // Set Mouse Wheel position to default

        // Still need to bounds check when recomputing (might cause interesting cases if scrolling between renders, maybe bounds check inside callback?)
        if(FoV>maximumFoV)
        {
            FoV=maximumFoV;
        }
        ...
        //ProjectionMatrix = glm::perspective(FoV, windowWidth()/windowHeight(), 0.1f, 100.0f); // Manually calculate the window ratio
        // Camera matrix
